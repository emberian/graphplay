...
/--
`FinerThan P` is a **lattice**: meets are the join of cell-labellings
...
-/
instance FinerThan.lattice
    {V : Type u} [Fintype V] [DecidableEq V] {G : WeightedGraph V}
    {I : Type u} [Fintype I] [DecidableEq I]
    (P : EquitablePartition G I) : Lattice (FinerThan P) := by
  sorry
...
Equivalently: the underlying-graph functor `WGraphP → WGraph` (`(G, P) ↦ G`)
has the discrete-partition functor as a **left adjoint** and the quotient
functor as a **right adjoint** (the coreflection / reflection in topos-jargon).

Statement-only.
-/
theorem discrete_adjoint_quotient :
    True := by
  -- Precise statement (deferred):
  -- `Discrete ⊣ Quotient` as functors `WGraphObj.{u} ⥤ WGraphPObj.{u}`
  -- vs. `WGraphPObj.{u} ⥤ WGraphObj.{u}`.
  trivial
...
Statement-only; the precise topology instance is deferred.
-/
theorem refinement_grothendieck_topology
    {V : Type u} [Fintype V] [DecidableEq V] (G : WeightedGraph V) :
    True := by
  -- Precise statement (deferred): there is `GrothendieckTopology (EPCat V G)`
  -- whose covers are refinement families that jointly refine each cell.
  trivial
